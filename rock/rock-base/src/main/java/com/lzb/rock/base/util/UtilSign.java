...
public class UtilSign {
	...
	public static boolean check(Object obj, String key, String sign) {
		...
		if (StringUtils.isBlank(sign)) {
			throw new BusException(RockEnum.PAEAM_ERR, "签名值不能为空");
		}

		if (StringUtils.isBlank(key)) {
			throw new BusException(RockEnum.PAEAM_ERR, "签名秘钥不能为空");
		}
		if (obj == null) {
			throw new BusException(RockEnum.PAEAM_ERR, "签名对象不能为空");
		}

		String newsign = sign(obj, key);

		if (StringUtils.isBlank(newsign)) {
			throw new BusException(RockEnum.SYSTTEM_ERR, "签名发生未知异常");
		}
